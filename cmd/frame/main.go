...
var modbusAddr = flag.String("modbus_addr", "192.168.0.1:1502", "The addr of the kostal modbus")
...
func renderForecast(w http.ResponseWriter, r *http.Request) {
	...
	forecastToday, err := solcast.GetForcast(today, *propertyID, *apiKey)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	forecastTomorrow, err := solcast.GetForcast(tomorrow, *propertyID, *apiKey)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func getPower() map[string]PowerItem {
	...
	forecast, err := solcast.GetForcast(today, *propertyID, *apiKey)
	if err != nil {
